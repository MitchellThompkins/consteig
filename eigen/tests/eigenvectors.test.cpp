...
TEST(eigenvectors, simple_symmetric)
{
    ...
    // This static constexpr evaluation proves the algorithm runs at compile-time
    ...
    // Eigenvector for 3 should be roughly [1/sqrt(2), 1/sqrt(2)] -> [0.707, 0.707] or [-0.707, -0.707]
    double v3_0 = V(0, idx_3).real;
    double v3_1 = V(1, idx_3).real;
    // They should be equal in magnitude and sign
    EXPECT_NEAR(consteig::abs(v3_0), consteig::abs(v3_1), kTol) << "V3 elements magnitude mismatch";
    ...
    EXPECT_NEAR(consteig::abs(v1_0), consteig::abs(v1_1), kTol) << "V1 elements magnitude mismatch";
    ...
    Complex<double> Av0 = Complex<double>{A(0, 0)} * V(0, 0) + Complex<double>{A(0, 1)} * V(1, 0);
    Complex<double> Av1 = Complex<double>{A(1, 0)} * V(0, 0) + Complex<double>{A(1, 1)} * V(1, 0);
...
TEST(eigenvectors, complex_rotation)
{
    ...
    for (Size j = 0; j < s; ++j)
    {
        ...
        // Av
        Complex<double> Av0 = Complex<double>{A(0, 0)} * V(0, j) + Complex<double>{A(0, 1)} * V(1, j);
        Complex<double> Av1 = Complex<double>{A(1, 0)} * V(0, j) + Complex<double>{A(1, 1)} * V(1, j);
        
        // lambda * v
        Complex<double> lv0 = lam * V(0, j);
        Complex<double> lv1 = lam * V(1, j);
        
        EXPECT_NEAR(Av0.real, lv0.real, kTol) << "Av = lv mismatch for eval " << j << " (row 0, real)";
        EXPECT_NEAR(Av0.imag, lv0.imag, kTol) << "Av = lv mismatch for eval " << j << " (row 0, imag)";
        EXPECT_NEAR(Av1.real, lv1.real, kTol) << "Av = lv mismatch for eval " << j << " (row 1, real)";
        EXPECT_NEAR(Av1.imag, lv1.imag, kTol) << "Av = lv mismatch for eval " << j << " (row 1, imag)";
    }
}
