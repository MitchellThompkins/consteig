...
template <typename T, Size S>
constexpr bool verify_values(const Matrix<Complex<T>, S, 1>& computed, const Matrix<Complex<T>, S, 1>& expected,
...
TEST(consteig_eigen, constexpr_eigenValues) {
    ...
    static constexpr Matrix<double, s, s> mat{{{{-4.4529e-01, 4.9063e+00, -8.7871e-01, 6.3036e+00},
...
TEST(consteig_eigen, non_symmetric_complex_eigenvalues) {
    ...
    // Static verify
    static constexpr Matrix<Complex<double>, s, 1> expected{{{{{{0.0, 1.0}}}, {{{0.0, -1.0}}}}}};
    static_assert(verify_values(eigenValueTest, expected, static_cast<double>(CONSTEIG_TEST_TOLERANCE)),
...
TEST(consteig_eigen, non_symmetric_general) {
    ...
    static constexpr Matrix<double, s, s> mat{{{{1.0, 2.0, 3.0}, {0.0, 1.0, 5.0}, {0.0, -2.0, 1.0}}}};
    ...
    for (const auto& v : vals) {
        if (std::abs(v.real - 1.0) < CONSTEIG_TEST_TOLERANCE) {
            if (std::abs(v.imag) < CONSTEIG_TEST_TOLERANCE)
                found_1 = true;
            else if (std::abs(v.imag - std::sqrt(10.0)) < CONSTEIG_TEST_TOLERANCE)
                found_c1 = true;
            else if (std::abs(v.imag + std::sqrt(10.0)) < CONSTEIG_TEST_TOLERANCE)
                found_c2 = true;
        }
