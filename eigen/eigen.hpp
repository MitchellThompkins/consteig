...
constexpr T wilkinsonShift(const T a, const T b, const T c) {
    T delta{(a - c) / 2};
    if (delta == static_cast<T>(0)) delta = consteig::epsilon<T>();
    ...
    while (n > 0 && total_iter < max_total_iter) {
        ...
        if (its > 0 && its % 10 == 0) {
            ...
            if (its % 20 == 0) {
                ...
                if (l + 2 <= n) sshift += consteig::abs(a(l + 2, l + 1));
            }
...
constexpr Matrix<Complex<T>, S, 1> eigvals(const Matrix<T, S, S> a) {
    ...
    T eps = consteig::epsilon<T>() * (norm1(out) + static_cast<T>(1.0));
    ...
    for (Size i = 0; i < S; ++i) {
        ...
        if (found_2x2) {
            ...
            if (disc >= 0) {
                ...
                result(i, 0) = Complex<T>{(tr + sq) / 2, 0};
                result(i + 1, 0) = Complex<T>{(tr - sq) / 2, 0};
            } else {
                T sq = consteig::sqrt(-disc);
                result(i, 0) = Complex<T>{tr / 2, sq / 2};
                result(i + 1, 0) = Complex<T>{tr / 2, -sq / 2};
            }
            i++;
        } else {
...
static inline constexpr bool checkEigenValues(const Matrix<T, R, C> a, const Matrix<Complex<T>, R, 1> lambda,
    ...
    if (consteig::abs(sum_lambda.real - tr) > thresh) return false;
    if (consteig::abs(sum_lambda.imag) > thresh) return false;

    if constexpr (R <= 4) {
        T d = det(a);
        Complex<T> prod_lambda{1, 0};
        for (Size i = 0; i < R; ++i) prod_lambda = prod_lambda * lambda(i, 0);
        T det_tol = thresh * (static_cast<T>(1) + consteig::abs(d));
        if (consteig::abs(prod_lambda.real - d) > det_tol) return false;
        if (consteig::abs(prod_lambda.imag) > det_tol) return false;
    }
