...
TEST(matrix, eigen_comparison)
{
    ...
    for (int i = 0; i < 3; ++i)
        for (int j = 0; j < 3; ++j)
            EXPECT_NEAR(sum(i, j), eigSum(i, j), CONSTEIG_TEST_TOLERANCE);
    ...
    for (int i = 0; i < 3; ++i)
        for (int j = 0; j < 3; ++j)
            EXPECT_NEAR(diff(i, j), eigDiff(i, j), CONSTEIG_TEST_TOLERANCE);
    ...
    for (int i = 0; i < 3; ++i)
        for (int j = 0; j < 3; ++j)
            EXPECT_NEAR(prod(i, j), eigProd(i, j), CONSTEIG_TEST_TOLERANCE);
    ...
    for (int i = 0; i < 3; ++i)
        for (int j = 0; j < 3; ++j)
            EXPECT_NEAR(trans(i, j), eigTrans(i, j), CONSTEIG_TEST_TOLERANCE);
