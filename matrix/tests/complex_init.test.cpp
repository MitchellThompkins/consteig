...
TEST(matrix, complex_init_minimal)
{
    ...
    // we need 5 levels of nesting to reach the elements:
    // 1. Matrix {
    // 2.   _data (Array<Array<T, C>, R>) {
    // 3.     _data[R] (The internal array of rows) {
    // 4.       Array<T, C> (The row object) {
    // 5.         _data[C] (The internal array of elements) {
    //            T1, T2, ...
    // 5.         }
    // 4.       }
    // 3.     }
...
{
    // For a 2x2 matrix, the 5 levels of braces are still required for each row.
