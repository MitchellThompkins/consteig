...
int main() {
    ...
    std::cout << "Cutoff: " << filter.fc << " Hz, Sampling Rate: " << filter.fs << " Hz" << std::endl;
    std::cout << "Continuous System Matrix A:" << std::endl;
    std::cout << "[ " << filter.A_c(0, 0) << ", " << filter.A_c(0, 1) << " ]" << std::endl;
    std::cout << "[ " << filter.A_c(1, 0) << ", " << filter.A_c(1, 1) << " ]" << std::endl;

    std::cout << "\nContinuous-time Poles (Eigenvalues of A):" << std::endl;
    std::cout << "p1 = " << filter.p1_c.real << " + j" << filter.p1_c.imag << std::endl;
    std::cout << "p2 = " << filter.p2_c.real << " + j" << filter.p2_c.imag << std::endl;

    std::cout << "\nDiscrete-time Poles (Mapped via z = exp(sT)):" << std::endl;
    std::cout << "z1 = " << filter.p1_d.real << " + j" << filter.p1_d.imag << std::endl;
    std::cout << "z2 = " << filter.p2_d.real << " + j" << filter.p2_d.imag << std::endl;
    ...
    std::cout << "y[n] = " << filter.K << " * x[n] - (" << filter.a1 << ") * y[n-1] - (" << filter.a2 << ") * y[n-2]" << std::endl;
