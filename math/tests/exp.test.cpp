...
TEST(exp_function, exp_one)
{
    ...
    static_assert(approxEqual(val, 2.718281828459, CONSTEIG_TEST_TOLERANCE),
                  MSG);
    ASSERT_TRUE(approxEqual(val, 2.718281828459, CONSTEIG_TEST_TOLERANCE));
}
...
TEST(exp_function, exp_negative)
{
    static constexpr double val = consteig::exp(-1.0);
    static_assert(
        approxEqual(val, 1.0 / 2.718281828459, CONSTEIG_TEST_TOLERANCE), MSG);
    ASSERT_TRUE(
        approxEqual(val, 1.0 / 2.718281828459, CONSTEIG_TEST_TOLERANCE));
}

TEST(exp_function, exp_large)
{
    static constexpr double val = consteig::exp(4.5);
    static_assert(approxEqual(val, 90.0171313005, CONSTEIG_TEST_TOLERANCE),
                  MSG);
    ASSERT_TRUE(approxEqual(val, 90.0171313005, CONSTEIG_TEST_TOLERANCE));
}

TEST(exp_function, exp_integer)
{
    static constexpr double val = consteig::exp(2);
    static_assert(approxEqual(val, 7.38905609893, CONSTEIG_TEST_TOLERANCE),
                  MSG);
    ASSERT_TRUE(approxEqual(val, 7.38905609893, CONSTEIG_TEST_TOLERANCE));
}
...
TEST(exp_function, exp_complex_pure_imaginary)
{
    ...
    static_assert(approxEqual(val.real, 0.0, CONSTEIG_TEST_TOLERANCE), MSG);
    static_assert(approxEqual(val.imag, 1.0, CONSTEIG_TEST_TOLERANCE), MSG);
...
TEST(exp_function, exp_complex_general)
{
    ...
    static_assert(
        approxEqual(val.real, 1.468693939915, CONSTEIG_TEST_TOLERANCE), MSG);
    static_assert(
        approxEqual(val.imag, 2.287355287178, CONSTEIG_TEST_TOLERANCE), MSG);
