...
TEST(exp_function, exp_one) {
    ...
    static_assert(compareFloats(val, 2.718281828459, kThresh), MSG);
    ASSERT_TRUE(compareFloats(val, 2.718281828459, kThresh));
}

TEST(exp_function, exp_negative) {
    static constexpr double val = consteig::exp(-1.0);
    static_assert(compareFloats(val, 1.0 / 2.718281828459, kThresh), MSG);
    ASSERT_TRUE(compareFloats(val, 1.0 / 2.718281828459, kThresh));
}

TEST(exp_function, exp_large) {
    static constexpr double val = consteig::exp(4.5);
    static_assert(compareFloats(val, 90.0171313005, kThresh), MSG);
    ASSERT_TRUE(compareFloats(val, 90.0171313005, kThresh));
}

TEST(exp_function, exp_integer) {
    static constexpr double val = consteig::exp(2);
    static_assert(compareFloats(val, 7.38905609893, kThresh), MSG);
    ASSERT_TRUE(compareFloats(val, 7.38905609893, kThresh));
}
